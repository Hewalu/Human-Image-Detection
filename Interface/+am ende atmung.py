...
# 3. BONUS PUNKTE (Die Überlauf-Punkte)
BONUS_LEDS_COUNT = 3     
...
def draw_led_ring(screen, active_leds, total_leds, state, breathing_alpha=255, is_slow_mode=False, pulse_factor=0.0):
    ...
    for i in range(total_leds):
        ...
        if state == STATE_GREEN:
            # 1. Reguläre LEDs (Die Schlange)
            leds_gone = total_leds - active_leds
            if i >= leds_gone: 
                is_lit = True
            
            # 2. Bonus LEDs (Die Verlängerung NACH 12 Uhr)
            # Das sind fix die Indizes 0, 1, 2 etc.
            if is_slow_mode:
                if i < BONUS_LEDS_COUNT:
                    is_bonus = True
                    is_lit = True # Bonus leuchtet immer, auch wenn Schlange voll ist
...
def main():
    ...
    while running:
        ...
        if current_state == STATE_TRAM:
            ...
            # Ring zeichnen (Bonus Punkte an Pos 0, 1, 2)
