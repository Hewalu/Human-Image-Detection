...
def draw_snail(screen, alpha):
    """Zeichnet die Schnecke mit Transparenz (Alpha), aber fester Größe"""
    snail_img = images.get('snail')
    if snail_img:
        # Kopie erstellen für Alpha
...
def draw_led_ring(screen, active_leds, total_leds, state, breathing_alpha=255, is_slow_mode=False):
    ring_center_y = CENTER_Y + OFFSET_RING_Y
    
    # Basisgröße (Bleibt jetzt immer fix!)
    current_dot_size = DOT_SIZE_BASE
    if total_leds > 60:
        factor = 60 / total_leds
        current_dot_size = max(2, int(DOT_SIZE_BASE * factor))
    
    # Surface Größe (Etwas Puffer)
...
def main():
    ...
    while running:
        ...
        # FADING-BERECHNUNG (Nur Transparenz)
        breath_alpha = 255
        
        if current_state == STATE_GREEN and slow_walker_detected:
             # Sanfte Sinuswelle für Fading
             pulse = (math.sin(now * 0.003) + 1) / 2 
             # Alpha Range: 80 bis 255 (Dunkel bis Hell)
             breath_alpha = int(80 + (175 * pulse)) 
        ...
        if current_state == STATE_TRAM:
            ...
            if timer_accumulator >= time_threshold:
                ...
                if current_state == STATE_GREEN:
                    led_counter -= 1
                    if led_counter < 0:
                        # ÜBERGANG ZU CLEARANCE:
                        # Wir übernehmen die Anzahl der Grün-Phase für die Räumzeit!
        ...
        if current_state == STATE_TRAM:
            ...
            # Hier nutzen wir current_total_leds (die Anzahl der Grünphase)
            ...
            # Schnecke zeichnen (auch nur Alpha Fading)
            if slow_walker_detected:
                draw_snail(screen, breath_alpha)
                
            # Ring zeichnen (atmet nur Alpha wenn Slow Mode)
            draw_led_ring(screen, led_counter, current_total_leds, STATE_GREEN, breath_alpha, is_slow_mode=slow_walker_detected)
