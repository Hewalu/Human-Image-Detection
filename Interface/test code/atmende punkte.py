...
def load_images():
    ...
    # KEIN ZENTRALES MÄNNCHEN MEHR LADEN
...
def draw_led_ring(screen, active_leds, total_leds, state, breathing_alpha=255, pulse_factor=0.0):
    ring_center_y = CENTER_Y + OFFSET_RING_Y
    
    # 1. Basisgröße berechnen
    base_size = DOT_SIZE_BASE
    if total_leds > 60:
        factor = 60 / total_leds
        base_size = max(2, int(DOT_SIZE_BASE * factor))
    
    # 2. ATEM-EFFEKT GRÖSSE (Floating Point Berechnung für Flüssigkeit)
    # Punkte wachsen um bis zu 40% ihrer Größe
    current_radius_float = base_size + (base_size * 0.4 * pulse_factor)
    current_radius = int(current_radius_float)
    
    # 3. SURFACE GRÖSSE (Buffer gegen abgeschnittene Kanten)
    # Wir machen die Surface deutlich größer als den Radius (Radius * 2 + 10px Buffer)
    ...
    for i in range(total_leds):
        ...
        if is_lit:
            ...
            # Surface für Alpha-Transparenz erstellen
            dot_surf = pygame.Surface((surf_size, surf_size), pygame.SRCALPHA)
            
            # WICHTIG: Wir zeichnen den Kreis in die Mitte der Surface.
            # Durch die transparente Surface und den Buffer bleibt der Kreis perfekt rund.
            pygame.draw.circle(dot_surf, current_color, (center_offset, center_offset), current_radius)
            
            # Wenn Atmen aktiv ist (durch Slow Mode oder Clearance), Alpha anwenden
            if breathing_alpha < 255 or pulse_factor > 0 or state == STATE_CLEARANCE:
                dot_surf.set_alpha(breathing_alpha)
            
            # Die Surface zentriert an die Position blitten
            screen.blit(dot_surf, (x_int - center_offset, y_int - center_offset))
        else:
            # Inaktive Punkte werden nicht gezeichnet (User Wunsch)
            pass 
...
def main():
    ...
    while running:
        ...
        if current_state == STATE_GREEN and slow_walker_detected:
             ...
             # Alpha schwankt zwischen 120 (transparent) und 255 (voll)
             breath_alpha = int(120 + (135 * pulse)) 
        ...
        if current_state == STATE_TRAM:
            ...
            # HIER WURDE DAS MÄNNCHEN ENTFERNT
            # Nur noch der Ring atmet, wenn Slow Mode aktiv
            draw_led_ring(screen, led_counter, current_total_leds, STATE_GREEN, breath_alpha, pulse_factor=pulse)
